...
public:
	...
	FORCEINLINE const float LengthSqr() const { return ((x * x) + (y * y) + (z * z)); }
	const Vector Normalized() const;
	void NormalizeInPlace();
	static const bool CompareTolerance(const Vector& vec0, const Vector& vec1, const float tolerance); // returns true if vectors are close enough
	static const bool CompareToleranceAbs(const Vector& vec0, const Vector& vec1, const float tolerance); // returns true if vectors are close enough
...
{
	assertm(a.IsValid(), "invalid vector");
	assertm(b.IsValid(), "invalid vector");
	assertm(&a != &result, "same vector for multiple params");
	assertm(&b != &result, "same vector for multiple params");
...
FORCEINLINE float VectorNormalize(Vector& vec)
{
	...
	const float invlen = FastRSqrtFast(sqrlen);
	vec.x *= invlen;
	vec.y *= invlen;
	vec.z *= invlen;

...
namespace ivps
{
	...
	// VERTEX ORDER (should apply to all verts, but does not?? skill issue)
	// 0 = 0
	// 1 = 2
	// 2 = -1
	inline void SourceFromIVPS(const compactpoint_t& point, Vector& out)
	{
		out[0] = point.pos[0];
		out[1] = point.pos[2];
		out[2] = -point.pos[1];

		out *= s_MetersToInches;
	}

	// VERTEX ORDER (skill issue, major spelling mistake)
	// 0 = 2
	// 1 = -1
	// 2 = 0
	inline void SourceFromIVPS_Silly(const compactpoint_t& point, Vector& out)
	{
		out[0] = point.pos[2];
		out[1] = -point.pos[1];
		out[2] = point.pos[0];
